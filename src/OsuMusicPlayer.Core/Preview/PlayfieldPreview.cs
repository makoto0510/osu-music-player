...
public static class PlayfieldPreviewBuilder
{
    ...
    internal static PlayfieldPreviewData Build(Beatmap beatmap)
    {
        ...
        var colours = beatmap.ColoursSection.ComboColours is { Count: > 0 } custom
            ? custom.Select(static colour => new PreviewComboColour(colour.R, colour.G, colour.B)).ToArray()
            : default_colours;
        ...
        foreach (var hitObject in beatmap.HitObjects.OrderBy(static hitObject => hitObject.StartTime))
        {
            ...
            if (ruleset is OsuRuleset.Osu or OsuRuleset.Catch)
            {
                ...
                {
                    comboNumber = 0;
                    comboColour = comboColour < 0 ? 0 : (comboColour + 1 + Math.Max(0, hitObject.ComboOffset)) % colours.Length;
                }
