...
最終更新: 2026-09-06(フェーズ 0 完了後)

## 現在の開発段階

**段階: documents.md の機能構想(基本機能・特殊機能)と基盤整備(フェーズ 0)を実装済み。残りは外部サービスや他 OS での実地確認と改善候補。**
...
- テスト: 122 件合格(Core 53 / Audio 18 / App 48 / Server 3)、警告ゼロ(TreatWarningsAsErrors)
...
| ビジュアル | 詳細ペイン(難易度一覧・星の色分け・プレビュー再生・osu! web リンク)、背景動画(libVLC、音声同期、mod 速度追従)、ストーリーボード(Skia 描画、加算合成、アニメーション、ヒットサウンドトリガー)、Theater モード |
...
- [ ] 設定 UI の整理(Settings パネルが横長になってきたためタブ化を検討)
