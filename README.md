...
詳細ペイン上部の「Hitsounds」を有効にすると、詳細ペインで選択中の難易度のヒットサウンド(サークル、スライダーの頭・折り返し・終点・ティック、スピナー終了)を音楽に同期して再生します。詳細ペインのヘッドホンアイコンを右クリック、またはツールパネルの「Hit sounds」タブの「Source」から、**譜面付属のヒットサウンド** (Beatmap) か **スキンのヒットサウンド** (Skin) かを切り替えられます。

サンプルは次の順で探します。
- **Beatmap 選択時:**
  1. 譜面フォルダー(カスタムインデックスやファイル名指定のサンプル)
  2. 選択中のスキン / osu!stable の設定ファイルで選択中のスキン(Windows のみ)
  3. osu!lazer に同梱の既定サンプル(`osu.Game.Resources.dll` を PE 形式として読み取るだけで、コードは実行しません)
- **Skin 選択時:**
  譜面付属のカスタム音をスキップし、選択中のスキン / osu!stable スキン / 既定サンプルから標準サウンドを再生します。
...
設定（Ctrl+,）→ Appearance → Interface / UI で、独立した画面構成の **Studio** と従来の **Classic** を切り替えられます。初期設定は Studio です。変更は即時反映され、再生状態を保ったまま次回起動時にも引き継がれます。配色テーマとアクセントカラーはどちらの UI でも利用できます。

Studio では Settings / Sources / Equalizer / Browse / Playlists を共通のツール領域に表示し、ツールを選ぶとそれまでのツールを閉じて選択したものだけを開きます。ツール領域を閉じてもキューの表示状態は変わりません。Ctrl+Q は Studio 専用キューの表示を切り替えます。Classic では従来どおり、Up Next キューを含む Now Playing ペイン全体を切り替えます。
